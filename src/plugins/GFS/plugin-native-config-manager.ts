...
interface BaseNativeConfig {
  id: string
  profileId: string
}
type NativeConfigItem = NativeConfigLocal | NativeConfigRemote

interface NativeConfigLocal extends BaseNativeConfig {
  type: 'local'
  configPath: string
}

interface NativeConfigRemote extends BaseNativeConfig {
  type: 'remote'
  configUrl: string
  cache?: Cache
}

type Cache = { enable: true; path?: string } | { enable: false }

interface NativeConfigManagerOptions {
  cacheDir: string
  managerPath: string
}

/** @type {EsmPlugin} */
export default (Plugin: PluginMetadata) => {
  ...
  /* 触发器 生成配置时 */
  const onGenerate = async (config: SingBoxConfig, profile: IProfile): Promise<SingBoxConfig> => {
    const cfg = manager.configs.value.find((c) => c.profileId === profile.id)
    if (!cfg) return config

    let nativeConfig: SingBoxConfig
    try {
      if (cfg.type === 'local') {
        const content = await Plugins.ReadFile(cfg.configPath)
        nativeConfig = JSON.parse(content) as SingBoxConfig
      } else {
        let content: string
  ...
  const openMainUI = (manager: NativeConfigManager) => {
    const { h, resolveComponent, defineComponent } = Vue

    const getProfileName = (cfg: NativeConfigItem) => {
      return (profilesStore.getProfileById(cfg.profileId) as IProfile | undefined)?.name ?? 'Not Found'
    }
    ...
    `,
      ...
      setup(_, { expose }) {
        ...
        return {
          ...
          editConfig: (cfg: NativeConfigItem) => {
            openEditModal(cfg, manager)
          },
          deleteConfig: async (cfg: NativeConfigItem) => {
  ...
  const openGuideModal = (manager: NativeConfigManager) => {
  ...
  const openEditModal = (cfg: NativeConfigItem, manager: NativeConfigManager) => {
    const { ref, defineComponent } = Vue
...
class NativeConfigManager {
  public readonly configs = Vue.ref<NativeConfigItem[]>([])
  private readonly cacheDir: string
  private readonly managerPath: string
  private readonly tempPath: string

  constructor(opts: NativeConfigManagerOptions) {
  ...
  public async init() {
    try {
      ...
      this.configs.value = JSON.parse(content) as NativeConfigItem[]
    } catch (error) {
  ...
  public async handleAddLocal(): Promise<boolean> {
    ...
    const sourceConfig = JSON.parse(content) as SingBoxConfig
    const profileName = file.name.replace(/\.json$/i, '')
    const profilesStore = Plugins.useProfilesStore()
    const profile = profilesStore.getProfileTemplate(profileName) as IProfile
    ...
    const newItem: NativeConfigItem = {
      id,
      type: 'local',
  ...
  public async handleAddRemote(url: string, cache: boolean): Promise<boolean> {
    if (!url.length) {
    ...
    try {
      ...
      const id = Plugins.sampleID()
      const handleCache = async (): Promise<Cache> => {
  ...
  public async validateConfig(content: string): Promise<boolean> {
    try {
      JSON.parse(content)
      await Plugins.WriteFile(this.tempPath, content)
      const isAlpha = (Plugins.useAppSettingsStore().app as { kernel: { branch: string } }).kernel.branch === 'alpha'
  ...
  public async deleteConfig(cfg: NativeConfigItem) {
  ...
  public async updateConfig(cfg: NativeConfigItem, options: { input: string; cache?: boolean | undefined }) {
  ...
  public async updateCache() {
    ...
    for (const cfg of configs) {
      const profileName = (Plugins.useProfilesStore().getProfileById(cfg.profileId) as IProfile | undefined)?.name ?? 'Not Found'
...
const selectLocalJsonFile = (): Promise<File | null> => {
...
const extractProfileNameFromUrl = (url: string): string => {
  const id = Plugins.sampleID()
