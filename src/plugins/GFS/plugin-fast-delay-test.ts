...
type Proxies = Exclude<UnpackArray<NonNullable<SingBoxConfig['outbounds']>>, { type: 'direct' | 'block' | 'selector' | 'urltest' }>

/** @type {EsmPlugin} */
export default (Plugin: PluginMetadata): PluginExposed => {
  const tmpPath = `data/.cache/${Plugin.id}`

  const testUrl = TEST_URL_MAP[Plugin['test_url'] as keyof typeof TEST_URL_MAP]
  const testTimeout: string = Plugin['test_timeout']
  ...
  const corePidMap = new Map<string, number>()

  const testDelay = async (subscription: Subscription) => {
    ...
    const proxies: Proxies[] = await Plugins.ReadFile(subscription.path)
      .then((c) => JSON.parse(c))
      .catch(() => [])
    ...
    const runtimeConfig: SingBoxConfig = {
      ...BASE_CONFIG,
      outbounds: proxies,
      experimental: {
    ...
    const corePid = (await runCore(testConfigPath)) as number
    ...
    const proxyUpdateMap = new Map<string, { newTag: string; delay: number }>()
    const newTagDelayMap = new Map<string, number>()

    const completedProxies: { ok: boolean; value: Proxies }[] = await Plugins.asyncPool(concurrencyLimit, proxies, async (proxy): Promise<Proxies> => {
      index += 1
    ...
    const finalProxiesToWrite = completedProxies
      .map((p) => p.value)
      .sort((a, b) => {
    ...
    await Plugins.WriteFile(subscription.path, JSON.stringify(finalProxiesToWrite, null, 2))
  ...
  const runCore = async (targetConfigPath: string) => {
    const isAlpha = (appSettingsStore.app as { kernel: { branch: string } }).kernel.branch === 'alpha'
  ...
  const stopCore = async (subId: string) => {
  ...
  const getProxyDelay = async (opts: DelayGetParams): Promise<number> => {
    ...
    try {
      ...
      return (body as { delay: number })?.delay ?? -1
    } catch (err) {
...
} as const satisfies SingBoxConfig
